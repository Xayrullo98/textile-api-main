...
def get_supplies(search: str = None,  id: int = 0,detail_id: int = 0,supplier_id: int = 0,currency_id = 0,  page: int = 1,
...
def supply_update(id: int, db: Session = Depends(database),
                    current_user: UserCurrent = Depends(get_current_active_user)):
    if delete_supply(id, current_user, db):
        raise HTTPException(status_code=200, detail="Amaliyot muvaffaqiyatli amalga oshirildi")