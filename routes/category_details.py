...
def add_category_detail(form: Category_detailsCreate, db: Session = Depends(database),current_user: UserCurrent = Depends(get_current_active_user)):
    # role_verification(current_user, inspect.currentframe().f_code.co_name)
    if create_category_detail(form=form, thisuser=current_user, db=db):
        raise HTTPException(status_code=200, detail="Amaliyot muvaffaqiyatli amalga oshirildi")


@category_details_router.get('/', status_code=200)
def get_category_details(search: str = None,  id: int = 0, measure_id: int = 0,category_id: int = 0, page: int = 1,
