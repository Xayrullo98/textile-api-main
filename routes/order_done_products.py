...
from functions.order_done_products import create_order_done_product, update_order_done_product,\
    all_order_done_products, one_order_done_product,create_order_done_product_for_mobile
...
@order_done_products_router.post('/add_products', )
def add__order_done_product_for_mobile(form: CreateOrder_done_products, db: Session = Depends(database),       ):
    create_order_done_product_for_mobile(form=form,  db=db)
    raise HTTPException(status_code=200, detail="Amaliyot muvaffaqiyatli amalga oshirildi")


@order_done_products_router.get('/all', status_code=200)
def get_order_done_products_for_mobile(stage_id: int = 0, order_id: int = 0,worker_id: int = 0,  id: int = 0, page: int = 1,
                    from_date: date = Query(None), to_date: date = Query(date.today()),
                    limit: int = 25, db: Session = Depends(database),
                                       ):
    if id:
